...
function apply(ctx, config) {
	ctx.inject(["settings"], (sctx) => {
		...
		const pendingChangeSession = /* @__PURE__ */ new Map();
		/** 提案目录内绑定标记文件的文件名。 */
		const MARKER_FILENAME = ".dsh-session";
		/** 读取提案目录的绑定标记；不存在/不可读返回 undefined。 */
		const readChangeMarker = async (changeDir) => {
			try {
		...
		const writeChangeMarker = async (changeDir, sessionId) => {
			try {
		...
		*/
		const reconcilePendingBindings = async (projects) => {
				...
				try {
					...
					switch (method) {
						...
						case "changeSession.bind": {
							...
							pendingChangeSession.set(projectPath, {
								sessionId,
								since: Date.now()
							});
							writeOk(res, {
						...
						case "changeSession.get": {
							...
							await reconcilePendingBindings([{
								path: projectPath,
								changes
							}]);
							writeOk(res, { sessionId: await readChangeMarker(join(projectPath, "openspec", "changes", changeName)) ?? null });
							return;
						}
